...
def merkleSnap(dir, snap):
    ...
    print(f"\nSignature and Time of Snapshot {snapIteration} has been published.")
    currTime = int(time.time())
    timePlusHash = currTime.to_bytes(64) + newHash
    signature = dss.sign(secret_key, timePlusHash)
    with open(f"{snap}/Signature + Time {snapIteration}.txt", "w") as f:
...
if __name__ == "__main__":
    ...
    snapName = input("\nEnter name for directory to store snapshots and signed hash digests in: ")
    ...
    with open(f"{snapName}/Public Key.txt", "w") as f:
